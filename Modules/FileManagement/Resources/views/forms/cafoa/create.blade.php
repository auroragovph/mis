...
<div class="row" id="app-root">
    <div class="col-md-12">
        <div class="card card-default px-5 py-3">
            <form action="{{ route('fms.cafoa.store') }}" method="POST">
                @csrf
                <h4>CAFOA Details</h4>
                <hr>
                ...
                <div class="form-group">
                    ...
                    <select name="requesting" class="form-control select2">
                        ...
                        @foreach($requestings as $requesting)
                            <option value="{{ $requesting->id }}">{{ name_helper($requesting->name) }}</option>
                        @endforeach
                    </select>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="budget" class="form-control select2">
                                ...
                                @foreach($budgets as $budget)
                                    <option value="{{ $budget->id }}">{{ name_helper($budget->name) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="accounting" class="form-control select2">
                                ...
                                @foreach($accountants as $accountant)
                                    <option value="{{ $accountant->id }}">{{ name_helper($accountant->name) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
