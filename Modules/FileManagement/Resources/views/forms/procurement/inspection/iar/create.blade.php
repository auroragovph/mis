...
@section('content')
<x-ui.card>
    <form action="{{ route('fms.procurement.iar.store') }}" method="POST">
        @csrf
       ...
       <div class="row">
           ...
           <div class="col-md-6">
                <x-ui.form.input label="AIR Number" name="number" />

                <div class="row">
                    <div class="col-md-6">
                        <x-ui.form.input label="Invoice Number" name="invoice" />
                    </div>
                    <div class="col-md-6">
                        <x-ui.form.input label="Date" type="date" name="invoice_date" />
                    </div>
                </div>
            </div>
       </div>
        ...
        @foreach($po->lists as $list)
            <div class="row">
                ...
                <div class="col-md-2">
                    <x-ui.form.input label="Receive Quantity" type="number" name="lists[]" required />
                </div>
            </div>
        @endforeach
